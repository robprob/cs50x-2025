...
person *create_family(int generations)
{
    ...
    // Return if no more available memory
    if (child == NULL)
    {
        return NULL;
    }
    ...
    if (generations > 1)
    {
        ...
        int randAllele0 = random() % 2;
        int randAllele1 = random() % 2;

        child->alleles[0] = parent0->alleles[randAllele0];
        child->alleles[1] = parent1->alleles[randAllele1];
    }
